...
(defn
  ...
  ([key] (let [state (state)]
           (when-not (contains? state key)
             (throw                                                               
                           (js/Error (str "Unable to find state with key: " key))))
           (get state key))))
...
(defn
  ...
  create-graphics. Converts args to floats."
         ...
         ([rgb] (.background (current-graphics) rgb))
                                                                                                  
         ([rgb alpha] (.background (current-graphics) rgb alpha))
...
(defn
  ...
  ([val]
           ...
           (.binary (current-applet) val))
  ([val num-digits]
                                                     
           (.binary (current-applet) val num-digits)))
...
(defn
  ...
  [amt low high]
         ...
         (.constrain (current-applet) amt low high))
...
(defn
  ...
  ([w h]
     (.createGraphics (current-applet) (int w) (int h)        :p2d))
  ([w h renderer]
...
(defn
  ...
  [radians]
         ...
         (.degrees (current-applet) radians))
...
(defn
  ...
  ([x1 y1 x2 y2]
           ...
           (.dist (current-applet) x1 y1 x2 y2))
  ([x1 y1 z1 x2 y2 z2]
                                                                                          
           (.dist (current-applet) x1 y1 z1 x2 y2 z2)))
...
(defn
  ...
  ([gray]
   (.fill (current-graphics) (float gray))
          (clear-no-fill-cljs (current-graphics)))
  ([gray alpha]
...
(defn
  ...
  [new-rate]
  (do
           (reset! (.-target-frame-rate (current-applet)) new-rate)
...
(defn
  ...
  ([val]
           ...
           (.hex (current-applet) val))
  ([val num-digits]
                                                  
           (.hex (current-applet) val num-digits)))
...
(defn
  ...
  [start stop amt]
         ...
         (.lerp (current-applet) start stop amt))
...
(defn
  ...
  ([a b]
           ...
           (.mag (current-applet) a b))
  ([a b c]
                                                     
           (.mag (current-applet) a b c)))
...
(defn
  ...
  [val low1 high1 low2 high2]
         ...
         (.map (current-applet) val low1 high1 low2 high2))
...
(defn
 ...
 (.noFill (current-graphics))
        (aset (current-graphics) no-fill-prop true))
...
(defn
  ...
  [val start stop]
         ...
         (.norm (current-applet) val start stop))
...
(defn
  ...
  [num exponent]
         ...
         (.pow (current-applet) num exponent))
...
(defn
  ...
  [degrees]
         ...
         (.radians (current-applet) degrees))
...
(defn
  ...
  []
         ...
         @(.-target-frame-rate (current-applet)))

(defn- no-fill?
  "Returns whether fill is disabled for current graphics."
  [graphics]
                               
         (true? (aget graphics no-fill-prop)))
...
(defn
  ...
  Also used to control the coloring of textures in 3D."
         ...
         ([rgb] (.tint (current-graphics) rgb))
         ([rgb alpha] (.tint (current-graphics) rgb alpha))
...
(defn
  ...
  [str-val]
         ...
         (.unbinary (current-applet) (str str-val)))
...
(defn
  ...
  [hex-str]
         ...
         (.unhex (current-applet) (str hex-str)))
...
;;;;;;;;;;;; This file autogenerated from src/cljx/quil/core.cljx
